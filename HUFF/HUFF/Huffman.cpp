...
{
	// Method that encodes a text file, and writes the results into a second file with extension .huf if no other extension is provided. This method will traverse the 8-bit ASCII text file
	// and create a Huffman Tree based on the number of occurences of a character within the text file. The tree is to have up to 256 roots to define each ASCII character that is avaialable within 
...
// uses the tree to write the byte data back out to an output file that will contain the original text.
	...
	while (!fin.eof())
	{
		...
		int binaryMult = 128;
		if ((int)fin.peek() == -1) break; // If an extra end of file character is read
		...
		for (int i = 0; i < 8; i++) // Go through every character in the bit string
		{
			...
			if (binaryMult > nextChar) // If the binary decimal representation at position i is still greater than the ASCII representation of the character
			{
				currentNode = currentNode->leftChild;  // Go left in the Huffman Tree if a zero is found
				binaryMult /= 2; // Go to the next position to check by dividing by 2
			}
			else
			{
				currentNode = currentNode->rightChild; // Else go right if the binary decimal representation at position i is less than
				nextChar = nextChar - binaryMult; // Take away that multiple from the ASCII interger value
				binaryMult /= 2;
			}
...
// to each ASCII character, and write it to the output file. There is a return status where, a 0 return value is a successful run, and anything else is an error. 
	...
	while (!fin.eof()) // While there are still characters left to write
	{
		...
		int EOFCheck = 0; // Used to check for EOF by indicating a -1 when the file is checked
		while (bitByte.length() < 8 && EOFCheck != -1)
		{
			unsigned char nextChar = (unsigned char)fin.get(); // Get the next char
			EOFCheck = (int)nextChar;  // Check if the EOF was read
			getBits = bitTable[(int)nextChar]; // Set the bit equivalent of the char with its encoded binary value
			for (int i = 0; i < getBits.length(); i++)
			{
				if (bitByte.length() != 8)	bitByte += getBits.at(i);         // While bitByte still doesn't have a byte, write to it
				else	                    overflowBitByte += getBits.at(i); // Otherwise, put the excess bits in the overflow bits
			}
		}

		while (bitByte.length() >= 8) // If bitByte still contains at least a byte
		{
			fout.put(bitsToChar(bitByte)); // Write the char equivalent of the bits out to the output file
			
			if (overflowBitByte.length() >= 8) // If the overflow bit string still has a byte
			{
				bitByte = overflowBitByte.substr(0, 8); // bitByte will capture the byte
				overflowBitByte = overflowBitByte.substr(8, overflowBitByte.length() - 8); // The overflow bits will capture the leftover bits
			}
			else
			{
				bitByte = overflowBitByte; // Else bitByte will be the leftover bits under 8 bits to have more bits tacked onto it
			}
...
// and return the padded char.
	...
	string paddingString = bitTable[255]; // Since this section of the bitTable is never used, get it's bit string for paddding
	int position = 0;

	while (bitByte.length() != 8) // Tack on these buffer bits until the string is finally a byte
	{
		bitByte += bitTable[255].at(position++);
	}
